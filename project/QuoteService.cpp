...
void QuoteService::InitTickStrategyConfig(SysConfig& sysConfig)
{
    ...
    for (auto contract : m_instrument)
    {
        ...
        dateFile =getFileName(dateDirPath.c_str());
        ...
        if (itr_mapProduct != mapProduct.end())
        {
                ...
                itr_mapModel++)
            {
                ...
                if (_access(pred_product_file.c_str(), 0) == -1)	//如果文件夹不存在
                {
                    std::cout << pred_product_file << "该文件夹不存在，准备创建" << std::endl;
                    _mkdir(pred_product_file.c_str());				//则创建
                }
                else
                {
                ...
                if (_access(pred_model_fiel.c_str(), 0) == -1)	//如果文件夹不存在
                {
                    std::cout << pred_model_fiel << "该文件夹不存在，准备创建" << std::endl;
                    _mkdir(pred_model_fiel.c_str());				//则创建
                }
                else
                {
                ...
                std::string predFile = pred_model_fiel + "/" + date + "_" + itr_mapModel->first + "_pred.csv";
...
void QuoteService::PushQue()
{
    ...
    while (1)
    {
       ...
       // NewTick(newData,rtData);
        ...
        {
            for (rsize_t i = 0; i < itr_mapSignalPtr->second.size(); i++)
            {
        ...
        {
            for (rsize_t i = 0; i < itr_maplinearPtr->second.size(); i++)
            {
...
std::string QuoteService::getTime()
{
...
void QuoteService::listModelFiles(const char* dir)
{
    ...
    do
    {
        if (findData.attrib & _A_SUBDIR) // 目录
        {
            ...
            std::string dirName = findData.name;
...
void QuoteService::listFiles(char* dir, std::string& dirName)
{
    modeConfigFile config;
    char dirNew[200];
    strcpy_s(dirNew, dir);
...
std::string QuoteService::getFileName(const char* dirPath)
{
    std::string dataName;
    char dirNew[200];
    strcpy_s(dirNew, dirPath);
    strcat_s(dirNew, "\\*.*"); //在目录后面加上"\\*.*"进行第一次搜索.
    ...
    handle = _findfirst(dirNew, &findData);
    if (handle == -1) // 检查是否成功
        std::cout << "getFileName error" << std::endl;
    do
    {
        if (findData.attrib & _A_SUBDIR) // 目录
        {
        ...
        {
            dataName = findData.name; // 打印文件的名称，并且后面可以打印文件的数目
            break;
        }
